...
export function EnhancedReportBuilder({
  ...
  const updateFilter = (index: number, updates: Partial<ReportConfig['filters'][0]>) => {
    const updatedFilters = config.filters.map((f, i) =>
      i === index ? { ...f, ...updates } : f
    );
  ...
  const updateAggregation = (
    index: number,
    updates: Partial<ReportConfig['aggregations'][0]>
  ) => {
  ...
  const handleSave = () => {
    if (!config.name.trim() || !config.dataSource.trim()) {
      showToast({
        message: 'Name and data source are required',
        type: 'error',
      });
      return;
    }
      ...
      <Card>
        ...
        <div className="space-y-3">
          ...
          {config.aggregations.length === 0 && (
            <p className="text-sm text-gray-500 text-center py-4">No aggregations added</p>
          )}
        </div>
      </Card>
