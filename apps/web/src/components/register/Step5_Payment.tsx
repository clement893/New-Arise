...
import { MVPCardForm } from './MVPCardForm';
...
export function Step5_Payment() {
  // MVP Mode: Use Stripe directly without Elements wrapper for pre-filled test card
  // To switch back to production mode with Stripe Elements, change this to false
  // and uncomment the Elements wrapper below
  const MVP_MODE = true;
  
  if (MVP_MODE) {
    // MVP Mode: Use Stripe directly without Elements
    return <PaymentFormContentMVP />;
  }
  
  // Standard Mode: Use Stripe Elements (for production)
  return (
    <Elements 
...
// MVP version that doesn't require Elements wrapper
function PaymentFormContentMVP() {
  const { planId, selectedPlan: storePlan, setStep } = useRegistrationStore();
  ...
  // MVP Mode: Pre-filled card form state
  const [cardNumber, setCardNumber] = useState('4242 4242 4242 4242');
  const [expiryDate, setExpiryDate] = useState('01/30');
  const [cvv, setCvv] = useState('565');
  ...
  // Load plan details (same as PaymentFormContent)
  useEffect(() => {
    const loadPlan = async () => {
      if (storePlan) {
        setSelectedPlan({
  ...
  // Render same UI as PaymentFormContent but with MVP form
  return (
    <div className="w-full max-w-4xl mx-auto p-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 rounded-lg p-8">
                ...
                <div className="relative">
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                      <p className="text-xs text-blue-800 font-medium mb-1">MVP Mode: Test Card (Pre-filled)</p>
