...
export default function UserPermissionsEditor({ userId, onUpdate }: UserPermissionsEditorProps) {
  const { permissions: allPermissions, customPermissions, loading: permissionsLoading, addCustomPermission, removeCustomPermission, loadPermissions } = useUserPermissions(userId);
  ...
  return (
    <div className="space-y-4">
      ...
      <Alert variant="info">
        <p className="text-sm">
          Les permissions custom permettent d'ajouter des permissions spécifiques à cet utilisateur au-delà de celles de ses rôles.
          Les permissions déjà accordées via les rôles ne sont pas affichées ici.
        </p>
      </Alert>
